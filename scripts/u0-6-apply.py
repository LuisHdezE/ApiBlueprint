...
def replace_between(start: str, end: str, replacement: str) -> None:
    ...
    if start_index < 0 or end_index < 0:
        raise SystemExit(f'Unable to locate method boundaries: {start!r} -> {end!r}')
    source = source[:start_index] + replacement + source[end_index:]


replace_once("            'phpunit.xml' => $this->phpUnitFile(),", "            'phpunit.xml' => $this->phpUnitFile($manifest),")
...
phpunit_method = r'''    private function phpUnitFile(array $manifest): string
    {
...
replace_once(
    ...
    product_methods + '    private function normalizeFile(string $path, string $content): string\n',
)
