...
{
    public function __construct(private LaravelFeatureRecipeRegistry $recipes) {}
    ...
    private function buildFiles(array $manifest): array
    {
        ...
        foreach ($this->recipes->files($manifest) as $path => $content) {
            if (array_key_exists($path, $files) && $files[$path] !== $content) {
                throw new RuntimeException('Feature recipe conflicts with core generated file: '.$path);
            }
    ...
    private function composerFile(array $manifest): string
    {
        ...
        foreach ($this->recipes->composerRequireDev($manifest) as $package => $constraint) {
...
PHP;
    ...
    {
        $controller = $this->recipes->controller($endpoint, $className);
        if ($controller !== null) {
            return $controller;
        }
...
PHP;
    ...
    private function serviceProviderFile(array $manifest): string
    {
        ...
        $imports = array_merge($imports, $this->recipes->serviceProviderImports($manifest));
        $registerLines = array_merge($registerLines, $this->recipes->serviceProviderRegistrations($manifest));
